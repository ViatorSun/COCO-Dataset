...
def main():
    ...
    parser.add_argument("--input_dir",  default='/Users/viatorsun/Desktop/Demo/Data/Tomato/NewJson2',help="input annotated directory")
    parser.add_argument("--output_dir", default='/Users/viatorsun/Desktop/Demo/Data/Tomato/COCOInstance3' , help="output dataset directory")
    ...
    data = dict(
        ...
        categories  = [ ],          # 类别 supercategory, id, name
    )
    ...
    for image_id, filename in enumerate(label_files):
        ...
        for instance, mask in masks.items():
            ...
            data["annotations"].append(dict(id=len(data["annotations"]), image_id=image_id, category_id=cls_id,
                                            segmentation=segmentations[instance], area=area, bbox=bbox, iscrowd=0 )   )

        if not args.noviz:
            viz = img
            if masks:
                labels, captions, masks = zip(
                    *[  (class_name_to_id[cnm], cnm, msk) for (cnm, gid), msk in masks.items() if cnm in class_name_to_id  ]   )
